...
export default function OptionsPage() {
  ...
  const mySlug = me ? memberDisplaySlug(me.displayName) : "";
  const myUrl =
    origin && mySlug
      ? `${origin}/join/${group.inviteToken}/${encodeURIComponent(mySlug)}`
      : "";
  ...
  async function copyMyLink() {
    ...
    setErr(null);
    if (!myUrl) {
      setErr("Personal link not ready.");
      return;
    }
    try {
      await navigator.clipboard.writeText(myUrl);
      setMsg("Copied your link.");
    } catch {
      setErr("Copy blocked. Select the URL manually.");
    }
  }
        ...
        {myUrl ? (
          <div className="space-y-2">
            <p className="text-xs text-zinc-600">
              Your personal link (opens your member on this device—same as{" "}
              <strong>Log back in</strong> with your name):
            </p>
            <button
              type="button"
              onClick={() => void copyMyLink()}
              className="border-2 border-zinc-900 bg-white px-4 py-2 text-xs font-semibold text-zinc-900 shadow-[2px_2px_0_0_#18181b]"
            >
              Copy my link
            </button>
            <div className="border-2 border-zinc-900 bg-zinc-50 p-3 shadow-[2px_2px_0_0_#18181b]">
              <p className="break-all font-mono text-xs text-zinc-900 select-all">
                {myUrl}
              </p>
            </div>
          </div>
