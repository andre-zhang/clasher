...
import { walkBandsForOrderedCalendarPlans } from "@/lib/planWalkBands";
  ...
  async function download() {
    ...
    try {
      ...
      for (const dayLabel of picked) {
        ...
        const walkBands =
          scope === "member" &&
          group.walkTimesEnabled &&
          slots.length >= 2
            ? walkBandsForOrderedCalendarPlans(group, slots)
            : undefined;
    ...
    <details className="rounded border-2 border-zinc-900 bg-white">
      ...
      <div className="space-y-3 border-t border-zinc-200 p-3">
        ...
        <button
          ...
          className="border-2 border-zinc-900 bg-violet-600 px-3 py-2 text-sm font-semibold text-white disabled:opacity-40"
        >
